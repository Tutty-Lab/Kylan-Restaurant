...
describe("Feiertage (Hamburg)", () => {
  ...
  it("enthält die festen und beweglichen Hamburg-Feiertage 2026", () => {
    ...
    expect(h.has("2026-10-31")).toBe(true); // Reformationstag (Hamburg seit 2018)
  ...
  it("enthält KEINE Feiertage anderer Bundesländer", () => {
    ...
    expect(h.has("2026-06-04")).toBe(false); // Fronleichnam – nicht in Hamburg
    expect(h.has("2026-11-01")).toBe(false); // Allerheiligen – nicht in Hamburg
    expect(h.has("2026-11-18")).toBe(false); // Buß- und Bettag – nur Sachsen
    expect(h.has("2026-01-06")).toBe(false); // Heilige Drei Könige – nicht in Hamburg
  });
